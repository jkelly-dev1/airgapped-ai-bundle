...
def spend_usd(spend, models) -> float:
    """Dollars billed so far, from the token counters the providers returned.

    The ceiling is checked against this, not against the estimate. A pre-flight
    estimate uses assumed output lengths; a model that runs long bills what it
    actually emitted. Checking only the estimate makes --max-cost a ceiling on
    a guess rather than on the bill."""
    return sum(spend[(m, "in")] / 1e6 * PRICING[m]["in"]
...
# responder to assert something the evidence cannot support: six of eight
# components never report an input age, so a green view is equally consistent
# with a fresh enclave and one months past budget. Measured offline over two
# years, 134 distinct assessor views occur in BOTH the clean and the silent
# stratum, covering 210 of 338 clean days and 186 of 324 silent days; the
# artifact literally cannot distinguish them.
#
...
def run_calls(models, days, inv, max_cost, client_for,
              call_for=None, echo=print):
    """Every paid call, with the ceiling enforced against what was BILLED.

    Separated from main() so it can be driven with a fake `call_for` and no
    API key. The ceiling must be checked against the bill and not the
    estimate: the estimate assumes a fixed output length per provider
    (OUTPUT_TOKENS), MAX_OUTPUT_TOKENS is more than four times that, and a
    model that runs long bills the difference.

    Returns (records, spend). Stops as soon as the running bill passes
    `max_cost`; calls already made are kept, because they were billed whether
    or not they are recorded.
    """
    if call_for is None:
        def call_for(model):
            return (call_anthropic if PROVIDER[model] == "anthropic"
                    else call_openai)
    ...
    for model in models:
        ...
        for stratum, day in days:
            ...
            for condition, prompt in (
                ...
                records.append(rec)
                if billed > max_cost:
                    echo(f"\nSTOPPING: ${billed:.2f} billed exceeds the "
                         f"${max_cost:.2f} ceiling. "
                         f"{len(records)} calls recorded, the rest skipped.")
                    over = True
                    break
            echo(f"  {model:<16} day {day.day:<4} {stratum}")
...
def main() -> int:
    ...
    if cost > args.max_cost:
        print(f"\nREFUSING TO START: estimated ${cost:.2f} exceeds "
              f"${args.max_cost:.2f}")
        return 2
    if not args.confirm:
    ...
    inv = inventory_text()
    t0 = time.time()
    records, spend = run_calls(args.models, days, inv, args.max_cost,
                               lambda m: clients[PROVIDER[m]])
    elapsed = time.time() - t0
    total = spend_usd(spend, args.models)
