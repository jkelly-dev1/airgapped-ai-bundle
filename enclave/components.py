...
to something a real enclave stack contains: a served model, a license check, a
policy bundle, a vulnerability feed, a retrieval corpus, a clock, and each
...
The thing being measured is silence. Air-gapping is usually discussed as "does
it still work?", and the answer is almost always yes, that is what caching is
for. The question that decides whether an enclave is trustworthy is narrower
and nastier:
...
that produces an accreditation nobody can defend, the only one worth building
a repository around.
