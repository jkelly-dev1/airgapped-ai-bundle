...
CADENCE_DAYS = {
    ...
    # of this file modeled as a daily transfer: a network dependency inside a
    # boundary defined by not having one. A clock in an enclave is disciplined
    # when somebody physically visits with a reference, which is a quarterly
    # event, and free-runs in between.
...
def summarize(timeline: list) -> dict:
    ...
    return {
        ...
        # anywhere in the stack was saying so: no warning, no degraded health,
        # no banner. An operator looking at every dashboard they have sees a
        # working system.
